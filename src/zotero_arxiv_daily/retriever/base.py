...
class BaseRetriever(ABC):
    ...
    def retrieve_papers(self) -> list[Paper]:
        ...
        max_workers = int(getattr(self.config.executor, "max_workers", 4) or 4)

        # 单线程兜底，方便排错
        if max_workers <= 1:
            papers = []
            for rp in tqdm(raw_papers, total=len(raw_papers), desc="Converting papers"):
                p = self.convert_to_paper(rp)
                if p is not None:
                    papers.append(p)
            return papers
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as exec_pool:
            futures = {
                exec_pool.submit(self.convert_to_paper, rp): i
